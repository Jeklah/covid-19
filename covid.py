...
class Person():
    def __init__(self, startingImmunity):
        self.immunity = random.randint(0, 100) < startingImmunity
...
def initiateSim():
    ...
    for _ in range(startingInfecters):
...
with open('pandemicSave.txt', 'a') as saveFile:
    for x in range(100):
        print('Day ', x)
        if x == lockdownDay:
            lockdown = True
